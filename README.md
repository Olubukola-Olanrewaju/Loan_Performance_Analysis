## Loan Risk and Performance Analysis Dashboard
## Overview

This project analyzes loan data to evaluate credit risk, loan performance, and customer repayment behavior. The goal is to generate actionable insights that support better lending decisions and reduce financial risk.

## Business Problem

Financial institutions face challenges in identifying high risk borrowers and managing loan defaults. Poor risk assessment can lead to increased losses and reduced profitability.

This project focuses on uncovering patterns in loan data to improve credit risk evaluation and repayment monitoring.

## Objectives
- Analyze loan distribution across customer segments
- Identify factors influencing loan default
- Evaluate repayment performance and trends
- Monitor key risk indicators
- Provide actionable recommendations for risk reduction
## Tools Used
- Power BI for data visualization and dashboard development
- Excel for data cleaning and preprocessing
## Data Preparation
- Cleaned and structured raw loan dataset in Excel
- Handled missing values and corrected inconsistent data types
- Created calculated columns to support analysis
## Data Modeling
- Built a structured data model in Power BI
- Created DAX measures for key performance metrics
- Designed interactive visuals for multi dimensional analysis
## Key Metrics
- Total Loan Amount
- Total Repayment
- Default Rate
- Recovery Rate
- Number of Borrowers
## Dashboard Structure
## Page 1

Loan Overview
Summary of total loans, repayments, and key performance indicators

## Page 2

Credit Risk Analysis
Breakdown of high risk segments and default patterns

## Page 3

Repayment Behavior
Analysis of repayment trends and customer performance

## Page 4

## Customer Segmentation
Insights into borrower categories and loan distribution

## Key Insights
- Certain customer segments exhibit higher default rates
- Loan amount and repayment behavior show strong correlation
- Some loan categories carry significantly higher risk
- Early repayment delays are strong indicators of potential default
## Recommendations

Based on the analysis, the following actions are recommended to improve loan performance and reduce credit risk

- Strengthen credit assessment for high risk customer segments
- Implement early warning systems to detect repayment delays
- Adjust loan terms for high risk categories to reduce exposure
- Reward reliable customers with strong repayment history
- Continuously monitor key risk metrics for better decision making
## Screenshots

()

## Conclusion

This project demonstrates how data can be used to identify risk patterns, improve loan performance, and support smarter lending decisions. The insights generated can help financial institutions reduce losses and enhance overall portfolio performance.
